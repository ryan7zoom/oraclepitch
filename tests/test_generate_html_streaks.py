...
Tests for the mismatch/streak sections added to
engine/output/generate_html.py. Kept separate from
test_generate_html.py (which covers the original predictions-table
rendering) since this is a distinct, substantial feature area.

Critical checks, per the spec's explicit rules:
...
- Backward compatibility: generate_html() must still work when called
  with just `predictions` and no mismatch/streak arguments.
"""
...
def test_generate_html_backward_compatible_with_no_streak_args():
    html = generate_html([])
    assert "<!DOCTYPE html>" in html
    assert "Mismatches Found" not in html
    print("PASS: test_generate_html_backward_compatible_with_no_streak_args")
...
def test_full_generate_html_integration_with_mismatches_and_streaks():
    ...
    html = generate_html([], all_mismatches=all_mismatches, all_streaks=all_streaks)
    ...
    open_tags = re.findall(r"<(div|table|tr|td|th|h2|h3)\b", html)
    close_tags = re.findall(r"</(div|table|tr|td|th|h2|h3)>", html)
...
if __name__ == "__main__":
    test_generate_html_backward_compatible_with_no_streak_args()
